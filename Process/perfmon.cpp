...
//
BOOL CPerfMon::InitPDHDLL()
{
	m_hPDHDLL = LoadLibrary( "pdh.dll" );
	if ( !m_hPDHDLL )
	{
		return TRUE;
	} 
...
BOOL CPerfMon::Initialize()
{
	...
	PdhOpenQuery = (PDHOPENQUERY)GetProcAddress( m_hPDHDLL, "PdhOpenQueryA"); 
...
void CPerfMon::Uninitialize()
{
	...
	PdhCloseQuery = (PDHCLOSEQUERY)GetProcAddress( m_hPDHDLL, "PdhCloseQuery"); 
...
int CPerfMon::AddCounter(const char *pszCounterName)
{
	...
	PDHADDCOUNTER PdhAddCounter;
    
	PdhAddCounter = (PDHADDCOUNTER)GetProcAddress( m_hPDHDLL, "PdhAddCounterA"); 

	// add to current query
	if (PdhAddCounter(m_hQuery, pszCounterName, (DWORD)pCounter, &(pCounter->hCounter)) != ERROR_SUCCESS)
	{
		//TRACE("PdhAddCounter:%d\n",GetLastError());
...
BOOL CPerfMon::RemoveCounter(int nIndex)
{
	...
	PdhRemoveCounter = (PDHREMOVECOUNTER)GetProcAddress( m_hPDHDLL, "PdhRemoveCounter"); 
...
BOOL CPerfMon::CollectQueryData()
{
	...
	PdhCollectQueryData = (PDHCOLLECTQUERYDATA )GetProcAddress( m_hPDHDLL, "PdhCollectQueryData"); 
...
BOOL CPerfMon::UpdateValue(PPDHCOUNTERSTRUCT pCounter)
{
	...
	PdhGetFormattedCounterValue = (PDHGETFORMATTEDCOUNTERVALUE )GetProcAddress( m_hPDHDLL, "PdhGetFormattedCounterValue"); 
...
BOOL CPerfMon::UpdateRawValue(PPDHCOUNTERSTRUCT pCounter)
{
	...
	PdhGetRawCounterValue = (PDHGETRAWCOUNTERVALUE )GetProcAddress( m_hPDHDLL, "PdhGetRawCounterValue"); 
...
BOOL CPerfMon::GetStatistics(long *nMin, long *nMax, long *nMean, int nIndex)
{
	...
	PdhComputeCounterStatistics = (PDHCOMPUTECOUNTERSTATISTICS )GetProcAddress( m_hPDHDLL, "PdhComputeCounterStatistics"); 
